...
export default async function RequestDetailPage({ params }: PageProps<"/requests/[id]">) {
  ...
  const [department_, files, commentAttachments, accountingRows, paymentRow, events, comments, queryRows, routedApproverName, flagContext] = await withGrantScope(
    session.userId,
    role,
    async (tx) => {
      ...
      return [dept, bills, attachments, accountingHistory, pay ?? null, eventRows, commentRows, queries, routedName, flagCtx];
    },
  );
      ...
      <header className="flex flex-col gap-4 rounded-xl border border-line bg-surface p-4">
        <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-3">
          ...
          <p className="text-right font-mono text-2xl font-medium tracking-tight tabular-nums">
            ...
            <span className="block font-sans text-[11.5px] font-normal tracking-normal text-ink-3">{req.currency}</span>
          </p>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
        ...
        <div className="flex flex-col gap-4">
          ...
          {(latestAccounting || paymentRow) && (
            <section className="flex flex-col gap-2 rounded-xl border border-line-soft bg-surface p-4">
              ...
              <dl className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-1.5 text-[13px]">
                ...
                {paymentRow && (
                  <>
                    ...
                    <dd className="m-0 text-right font-mono tabular-nums">{formatMinorUnits(paymentRow.amountMinor, req.currency)}</dd>
                    {paymentRow.tdsMinor > 0 && (
                      <>
                        <dt className="text-ink-3">TDS</dt>
                        <dd className="m-0 text-right font-mono tabular-nums">{formatMinorUnits(paymentRow.tdsMinor, req.currency)}</dd>
                      </>
                    )}
                    <dt className="text-ink-3">{paymentRow.mode.toUpperCase()}</dt>
                    <dd className="m-0 text-right font-mono">{paymentRow.valueDate}</dd>
                    <dt className="text-ink-3">UTR</dt>
                    <dd className="m-0 text-right font-mono wrap-anywhere">{paymentRow.reference}</dd>
                  </>
                )}
              </dl>
            </section>
